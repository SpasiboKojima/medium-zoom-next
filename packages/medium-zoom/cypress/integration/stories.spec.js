...
const storybook = [
	{
		kind: 'attributes',
		stories: [{ name: 'data-zoom-src' }, { name: 'srcset' }, { name: 'srcset and data-zoom-src' }, { name: 'SVG' }],
	},
	{
		kind: 'options',
		stories: [
		...
		category.stories.forEach((story) => {
			it(story.name, () => {
				...
				cy.findInStory('.medium-zoom-image').each(($image, index) => {
					...
					cy.matchImageSnapshot(`${category.kind}/${[story.name.toLowerCase().replace(/\s/g, '-'), index + 1].join('-')}`, {
