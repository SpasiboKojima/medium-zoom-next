...
describe('update()', () => {
	...
	test('update({ container: Element }) updates options', () => {
		const zoom = mediumZoom();
		const initialOptions = zoom.getOptions();
		const updatedOptions = zoom.update({ container: document.body }).getOptions();
...
describe('open()', () => {
	...
	test('open() with `srcset` renders correctly', async () => {
		...
		image.src = 'image.jpg';
		image.srcset =
			'image-300x200.jpg 300w, image-600x400.jpg 600w, image-800x500.jpg 800w, image-1000x650.jpg 1000w, image-1200x800.jpg 1200w';
		root.appendChild(image);
...
describe('on()', () => {
	...
	test('"close" is called', async () => {
		...
		expect(onClose).toHaveBeenCalledWith(expect.objectContaining({ target: image, detail: { zoom } }));
	});
	...
	test('"closed" is called', async () => {
		...
		expect(onClosed).toHaveBeenCalledWith(expect.objectContaining({ target: image, detail: { zoom } }));
	});
	...
	test('"update" is called', () => {
		...
		expect(onUpdate).toHaveBeenCalledWith(expect.objectContaining({ target: image, detail: { zoom } }));
	});
	...
	test('"detach" is called for detach(image)', () => {
		...
		expect(onDetach).toHaveBeenCalledWith(expect.objectContaining({ target: image, detail: { zoom } }));
	});
	...
	test('"detach" is called for detach()', () => {
		...
		expect(onDetach).toHaveBeenNthCalledWith(1, expect.objectContaining({ target: image1, detail: { zoom } }));
		expect(onDetach).toHaveBeenNthCalledWith(2, expect.objectContaining({ target: image2, detail: { zoom } }));
	});
